...
from clients.utils.api_decorators import measure_latency
from clients.utils.api_decorators import rate_limiter


class BaseClient(Exchange):
    ...
    @measure_latency
    def get_exchange_info(self):  # todo: must to be standartizated
    ...
    def check_server_time(self):
        return self.get_server_time()['serverTime']
    ...
    @measure_latency
    def check_order(self, id, symbol, **kwargs):
        return self.fetch_order(id, symbol, kwargs)
    ...
    @measure_latency
    def cancel(self, id, symbol, **kwargs):
        return self.cancel_order(id, symbol, params=kwargs)

    @measure_latency
    def get_order_book(self, symbol, length=None, **kwargs):
        return self.fetch_order_book(symbol, length, **kwargs)

    @measure_latency
    def get_active_orders(self, symbol=None, since=None, limit=None, params={}, *args, **kwargs):
        return self.fetch_open_orders(symbol, since, limit, params, *args, **kwargs)

    @measure_latency
    def get_orders(self, symbol=None, since=None, limit=None, params={}, *args, **kwargs):
        return self.fetch_open_orders(symbol, since, limit, params, *args, **kwargs)

    @measure_latency
    def get_open_orders(self, symbol=None, since=None, limit=None, **kwargs):
        return self.fetch_open_orders(symbol, since, limit, params=kwargs)

    @measure_latency
    def get_closed_orders(self, symbol=None, since=None, limit=None, **kwargs):
        return self.fetch_closed_orders(symbol, since, limit, params=kwargs)

    @measure_latency
    def balance(self, **kwargs):
        return self.fetch_balance(**kwargs)

    @measure_latency
    def get_trades(self, symbol, **kwargs):
        return self.fetch_trades(symbol, params=kwargs)
    ...
    @measure_latency
    def get_all_tickers(self, **kwargs):
        if self.has['fetch_tickers']:
            return self.fetch_tickers(**kwargs)
        else:
            raise Exception("Exchange doesn't support such function: %s" % 'fetch_tickers')  # PUT THIS IN DECORATOR

    @measure_latency
    def get_ohlcv(self, symbol, period, **kwargs):
        if self.has['fetch_ohlcv']:
            return self.fetch_ohlcv(symbol, period, **kwargs)
        else:
            raise Exception("Exchange doesn't support such function: %s" % 'get_ohlcv')

    # WSS methods
    def ws_loop_order_book(self, *args, **kwargs):
        self.ws_client.ws_loop_order_book(*args, **kwargs)

    def ws_loop_ticker(self, *args, **kwargs):
        self.ws_client.ws_loop_ticker(*args, **kwargs)

    def ws_loop_tickers(self, *args, **kwargs):
        self.ws_client.ws_loop_tickers(*args, **kwargs)

    def ws_loop_balance(self, *args, **kwargs):
        self.ws_client.ws_loop_balance(*args, **kwargs)

    def ws_loop_candles(self, *args, **kwargs):
        self.ws_client.ws_loop_candles(*args, **kwargs)
